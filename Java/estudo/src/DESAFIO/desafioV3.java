...
public class desafioV3 {
	...
	public static void main(String[] args) {
		...
		// Escolha de um produto + loop de continuar S/N
		...
		while (opcao == 1) {
			...
			/*
			 * for (x = 0; x < produtos.length; x++) { } for (x = 0; x < quantidade.length;
			 * x++) { } for (x = 0; x < preco.length; x++) { }
			 */
			if (quantidadex > quantidade[codigo1 + 1]) {
				System.out.println(
						"\nVocê escolheu uma quantidade que não está dispónivel no estoque. Por favor selecione um valor correspondendo ao que temos disponível. ");
			...
			if (quantidadex <= quantidade[codigo1 + 1]) {
				...
				System.out.printf("O produto escolhido foi: %s ", produtos[codigo1 - 1]);
		...
		if (opcao1 == 1) { //Loop da mudança de compra
			while (opcao1 == 1) {
				...
				if (quantidadex > quantidade[codigo2 + 1]) { // digita se a quantidade não estiver no estoque
					System.out.println(
							"\nVocê escolheu uma quantidade que não está dispónivel no estoque. Por favor selecione um valor correspondendo ao que temos disponível. ");
					System.out.printf("\nPor favor %s, digite a quantidade de peças que você de deseja comprar: ",nome);
					quantidadex = leia.nextInt();
				...
				if (quantidadex <= quantidade[codigo2 + 1]) {
					quantidade[codigo2 + 1] = quantidade[codigo2 - 1] - quantidadex;
					System.out.printf("O produto escolhido foi: %s ", produtos[codigo2 - 1]);
					carrinho.add(produtos[codigo2 - 1]);
				}
				
				if (quantidadex == 1) { // plural ou singular
					System.out.printf("O produto escolhido foi: %s e sua quantidade é de %d peça ",
							produtos[codigo2 - 1], quantidade[quantidadex - 1]);
				} else if (quantidadex > 1) {
					System.out.printf("O produto escolhido foi: %s e sua quantidade é de %d peças ",
							produtos[codigo2 - 1], quantidade[quantidadex - 1]);
				}
				
				carrinho.add(produtos[codigo2 - 1]); // adiciona a compra no carrinho
				// calculo do preço
				subtotal = ((preco[codigo2 - 1] * quantidadex) + subtotal);
				
				System.out.printf("\nDeseja escolher outro produto, digite 1 para SIM ou 2 para NÃO? ");
				opcao1 = leia.nextInt(); // continua no loop sim ou não
			}
